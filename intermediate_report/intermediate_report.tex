...
As for running experiments, we run the training and test process 30 times, which means all the values referenced above, are the average of those values for 30 training and test sessions.
...
\section{Experiments}
The objective of the experiments is to identify the optimal neural net structure  and to identify which inputs really affect the output. This is done so that the neural network trains in the least amount of time and the training results are the best possible with the current data set available.

Despite selection of the structure and the inputs being the main goal, it is possible to test different values for the maximum number of iterations, error threshold, learning rate and momentum that can lead to better results but also create problems such as positive feedback loops and over fitting. To avoid these problems the values of said parameters need to be chosen carefully else the test results will not have a acceptable accuracy.

The experiments will be done separately for the math students data set and for the Portuguese students data set, as the data is from different students.
\subsection{Math}
