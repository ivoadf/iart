...
\end{tabular}
\newpage
\begin{tabular}{| c | p{1.5cm} | c | c | p{3cm} | c | c | c |}
\hline \textbf{min\_thresh} & \textbf{hidden} & \textbf{learn\_rate} & \textbf{momentum} & \textbf{removed} & \textbf{iterations} & \textbf{PassAcc} & \textbf{GradeAcc}\\
...
We will keep using the network with least hidden layers and nodes.
...
\hline 0.01 & \{1\} & 0.9 & 0.9 & \_sex \_age \_address \_reason \_paid \_activities \_nursery \_romantic \_freetime \_goout \_Walc \_health \_G1 \_G2 \_ & 241 & 87\% & 44\%\\
\hline
\end{tabular}
\caption{Momentum and Learning Rate Analysis}
\end{table}

Once again, as expected by our explanation of learning rate and momentum values in the first sections of this report and the analysis for learning rate and momentum values for the Maths data set,
\begin{itemize}
\item low learning rates, make for slow training.
\item high learning rates with low momentum lead to overfitting
\item high learning rates and high momentum do not lead to overfitting, and make for fast training. This happens for this problem. High learning rates might lead to overfitting in different problems.
...
An improvement that would make sense in this project would be to get a larger data-set. 
