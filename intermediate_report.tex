...
This means that $w_2$ depends on $z$ and $y$. As for $w_1$, it depends on $w_2, x$ and $y$ this means that calculating the error for one of the weights is a local computation. This gives the back-propagation algorithm its name. We propagate the error, $\frac{\partial P}{\partial w_2}$, backwards, in order to calculate $\Delta \overrightarrow{w}$, the vector containing the error for each weight.
...
The framework we will be using restricts the inputs and outputs to be in the range [0-1], so to train and test our network we need to first parse the data and transform all the inputs into the corresponding value in the rage [0-1].
\hfill \break
\hfill \break
